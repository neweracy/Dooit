import { StyleProp, TextStyle, View, ViewStyle, TouchableOpacity, ScrollView } from "react-native"
        ...
        >
          ...
          {showTasks && dayTasks.length > 0 && (
            <View style={themed($taskIndicatorContainer)}>
              {hasPendingTasks && <View style={themed($taskIndicatorPending)} />}
              {hasCompletedTasks && <View style={themed($taskIndicatorCompleted)} />}
            </View>
          )}
        </TouchableOpacity>
      )
      ...
      {showTasks && selectedDate && (
        <View style={themed($taskSummary)}>
          ...
          {monthTasks[selectedDate.getDate().toString()]?.length > 0 ? (
            <ScrollView style={themed($tasksList)} showsVerticalScrollIndicator={false}>
              {monthTasks[selectedDate.getDate().toString()].map((task, index) => (
                <View key={task.id || index} style={themed($taskItem)}>
                  ...
                  <View style={themed($taskContent)}>
                    ...
                    {task.description && (
                      <Text style={themed($taskDescription)}>{task.description}</Text>
                    )}
                  </View>
                </View>
              ))}
...
const $taskSummaryTitle: ThemedStyle<TextStyle> = ({ colors, typography, spacing }) => ({
  fontFamily: typography.primary.medium,
  fontSize: 16,
  color: colors.text,
